    ...
    const fetchBusinessData = async () => {
      try {
        ...
        if (response.ok) {
          ...
          if (!data.business.isPublished) {
            // Redirect to home if not published
            router.push('/');
            return;
          }

          // Ensure default structures are present
          if (!data.business.galleryUrls) data.business.galleryUrls = [];
          if (!data.business.services) data.business.services = [];

          setSiteData(data.business);
          setLoading(false);
        } else if (response.status === 404) {
          // Business not found in database - redirect to home
          console.log('Business not found, redirecting to home');
          router.push('/');
        } else {
          ...
          if (fallback) {
            ...
            // Redirect to home if no fallback
            router.push('/');
          }
        }
      } catch (error) {
